...
static char *construct_coinbase_tx(const char *coinbase_1, const char *coinbase_2,
                                   const char *extranonce, const char *extranonce_2)
{
    size_t coinbase_1_len = strlen(coinbase_1);
    size_t coinbase_2_len = strlen(coinbase_2);
    size_t extranonce_len = strlen(extranonce);
    size_t extranonce_2_len = strlen(extranonce_2);
    char *coinbase_tx = malloc(coinbase_1_len + coinbase_2_len + extranonce_len + extranonce_2_len + 1);

    TEST_ASSERT_NOT_NULL(coinbase_tx);

    memcpy(coinbase_tx, coinbase_1, coinbase_1_len);
    memcpy(coinbase_tx + coinbase_1_len, extranonce, extranonce_len);
    memcpy(coinbase_tx + coinbase_1_len + extranonce_len, extranonce_2, extranonce_2_len);
    memcpy(coinbase_tx + coinbase_1_len + extranonce_len + extranonce_2_len, coinbase_2, coinbase_2_len);
    coinbase_tx[coinbase_1_len + coinbase_2_len + extranonce_len + extranonce_2_len] = '\0';

    return coinbase_tx;
}

static char *calculate_merkle_root_hash(const char *coinbase_tx_hex, const uint8_t merkles[][32], int num_merkles)
{
    size_t coinbase_len = strlen(coinbase_tx_hex) / 2;
    uint8_t *coinbase_bin = malloc(coinbase_len);
    uint8_t merkle_root_bin[32];
    char *merkle_root_hex = malloc(65);

    TEST_ASSERT_NOT_NULL(coinbase_bin);
    TEST_ASSERT_NOT_NULL(merkle_root_hex);

    hex2bin(coinbase_tx_hex, coinbase_bin, coinbase_len);
    calculate_merkle_root_hash_bin(coinbase_bin, coinbase_len, merkles, num_merkles, merkle_root_bin);
    bin2hex(merkle_root_bin, sizeof(merkle_root_bin), merkle_root_hex, 65);

    free(coinbase_bin);
    return merkle_root_hex;
}

static bm_job construct_bm_job(mining_notify *params, const char *merkle_root_hex, uint32_t version_mask, uint32_t difficulty)
{
    uint8_t merkle_root_bin[32];

    hex2bin(merkle_root_hex, merkle_root_bin, sizeof(merkle_root_bin));
    return construct_bm_job_bin(params, merkle_root_bin, version_mask, difficulty);
}

static void set_prev_block_hash(mining_notify *params, const char *prev_block_hash_hex)
{
    hex2bin(prev_block_hash_hex, params->prev_block_hash_bin, sizeof(params->prev_block_hash_bin));
    flip32bytes(params->prev_block_hash, params->prev_block_hash_bin);
    memcpy(params->prev_block_hash_be, params->prev_block_hash_bin, sizeof(params->prev_block_hash_be));
    reverse_bytes(params->prev_block_hash_be, sizeof(params->prev_block_hash_be));
}

static void word_reverse_32(const uint8_t *src, uint8_t *dest)
{
    for (int i = 0; i < 8; i++) {
...
// Values calculated from the bundled stratum job construction verifier.
TEST_CASE("Validate bm job construction", "[mining]")
{
    mining_notify notify_message = {0};
    ...
    set_prev_block_hash(&notify_message, "bf44fd3513dc7b837d60e5c628b572b448d204a8000007490000000000000000");
    
    // [修复 Bug 1]：增加 difficulty 参数（传入 0 作为测试占位符）
    bm_job job = construct_bm_job(&notify_message, merkle_root, 0, 0);

    uint8_t merkle_root_bin[32];
    uint8_t merkle_root_be[32];

    hex2bin(merkle_root, merkle_root_bin, sizeof(merkle_root_bin));
    word_reverse_32(merkle_root_bin, merkle_root_be);

    TEST_ASSERT_EQUAL_HEX32(0x20000004, job.version);
    TEST_ASSERT_EQUAL_HEX32(0, job.version_mask);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(merkle_root_bin, job.merkle_root, 32);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(merkle_root_be, job.merkle_root_be, 32);
    TEST_ASSERT_EQUAL_HEX32(notify_message.target, job.target);
    TEST_ASSERT_EQUAL_HEX32(notify_message.ntime, job.ntime);
}

TEST_CASE("Version submit bits preserve masked base ones without XOR folding", "[mining]")
{
    uint32_t base_version = 0x2000A004;
    uint32_t version_mask = 0x0000E000;
    uint32_t version_bits = 0x00004000;
    uint32_t rolled_version = rolled_version_from_bits(base_version, version_mask, version_bits);

    TEST_ASSERT_EQUAL_HEX32(0x20004004, rolled_version);
    TEST_ASSERT_EQUAL_HEX32(version_bits, submit_version_bits(rolled_version, version_mask));
}

TEST_CASE("BM job population assigns the full negotiated version mask to the ASIC", "[mining]")
{
    mining_notify notify_message = {0};
    bm_job job_template = {0};
    bm_job job = {0};
    uint8_t merkle_root_bin[32] = {0};
    uint8_t merkle_root_be[32];

    notify_message.version = 0x2001E004;
    notify_message.target = 0x1705dd01;
    notify_message.ntime = 0x64658bd8;
    for (uint8_t i = 0; i < sizeof(merkle_root_bin); i++) {
        merkle_root_bin[i] = i;
    }

    prepare_bm_job_template_into(
        &job_template,
        &notify_message,
        0x0001E000,
        1);
    ...
    TEST_ASSERT_EQUAL_HEX32(0x20000004, job.version);
    TEST_ASSERT_EQUAL_HEX32(0x0001E000, job.version_mask);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(merkle_root_bin, job.merkle_root, 32);
    TEST_ASSERT_EQUAL_UINT8_ARRAY(merkle_root_be, job.merkle_root_be, 32);
}

TEST_CASE("Exact pool target overrides integer difficulty approximation", "[mining]")
{
    mining_notify notify_message = {0};
    bm_job job = {0};

    notify_message.pool_target_set = true;
    for (size_t i = 0; i < sizeof(notify_message.pool_target); i++) {
        notify_message.pool_target[i] = (uint8_t)(i * 7U + 3U);
    }
    ...
    TEST_ASSERT_EQUAL_UINT8_ARRAY(notify_message.pool_target,
                                  job.pool_target,
                                  sizeof(job.pool_target));
}

TEST_CASE("Recent share cache suppresses duplicate share submissions", "[mining]")
{
    recent_share_cache cache = {0};

    TEST_ASSERT_TRUE(recent_share_cache_add(&cache, "job-1", "00000001", 0x12345678, 0x89ABCDEF, 0x00004000, 7));
    TEST_ASSERT_FALSE(recent_share_cache_add(&cache, "job-1", "00000001", 0x12345678, 0x89ABCDEF, 0x00004000, 7));
    TEST_ASSERT_TRUE(recent_share_cache_add(&cache, "job-1", "00000001", 0x12345678, 0x89ABCDEF, 0x00006000, 7));
    TEST_ASSERT_TRUE(recent_share_cache_add(&cache, "job-1", "00000001", 0x12345678, 0x89ABCDEF, 0x00004000, 8));
}

TEST_CASE("Clean jobs notify invalidates work even with empty software queues", "[mining]")
{
    TEST_ASSERT_TRUE(should_clean_jobs(true));
    TEST_ASSERT_FALSE(should_clean_jobs(false));
}
...
TEST_CASE("Test nonce diff checking", "[mining test_nonce][not-on-qemu]")
{
    ...
    set_prev_block_hash(&notify_message, "d02b10fc0d4711eae1a805af50a8a83312a2215e00017f2b0000000000000000");
...
TEST_CASE("Test nonce diff checking 2", "[mining test_nonce][not-on-qemu]")
{
    ...
    set_prev_block_hash(&notify_message, "0c859545a3498373a57452fac22eb7113df2a465000543520000000000000000");
