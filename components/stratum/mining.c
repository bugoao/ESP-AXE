                            ...
                            uint32_t work_epoch)
{
    ...
    for (size_t i = 0; i < RECENT_SHARE_CACHE_SIZE; i++) {
        if (recent_share_entry_matches(&cache->entries[i], jobid, extranonce2,
                                       ntime, nonce, version_bits, work_epoch)) {
            return false;
        }
    }
                                  ...
                                  uint32_t version_mask, uint32_t difficulty)
{
    ...
    job->pool_target_tracks_channel = params->pool_target_tracks_channel;

    memcpy(job->prev_block_hash, params->prev_block_hash, 32);
    memcpy(job->prev_block_hash_be, params->prev_block_hash_be, 32);

    if (params->pool_target_set) {
        memcpy(job->pool_target, params->pool_target, sizeof(job->pool_target));
    } else {
...
void populate_bm_job_from_template_into(bm_job *job, const bm_job *job_template,
