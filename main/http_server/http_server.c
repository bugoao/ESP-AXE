...
static bool json_u16_in_range(cJSON *item, const char *label,
                              uint16_t min_value, uint16_t max_value,
                              uint16_t *out)
{
    double raw_value;
    uint16_t clamped_value;
    bool value_was_clamped = false;
    ...
    if (cJSON_IsBool(item)) {
        if (min_value != 0U || max_value != 1U) {
            return false;
        }
        raw_value = cJSON_IsTrue(item) ? 1.0 : 0.0;
    ...
    if (raw_value != raw_value) {
        ESP_LOGW(TAG, "Ignoring NaN value for %s", label);
        return false;
    }

    if (raw_value < (double)min_value) {
        clamped_value = min_value;
        value_was_clamped = true;
    } else if (raw_value > (double)max_value) {
        clamped_value = max_value;
        value_was_clamped = true;
    } else {
        clamped_value = (uint16_t)raw_value;
        value_was_clamped = ((double)clamped_value != raw_value);
    }

    if (value_was_clamped) {
        ESP_LOGW(TAG, "Clamped %s from %.0f to %u", label, raw_value, (unsigned int)clamped_value);
    }
...
static void nvs_set_u16_json_clamped(cJSON *root, const char *json_key, const char *nvs_key,
                                     uint16_t min_value, uint16_t max_value)
{
    cJSON *item = cJSON_GetObjectItem(root, json_key);
    uint16_t value;

    if (json_u16_in_range(item, json_key, min_value, max_value, &value)) {
        nvs_config_set_u16(nvs_key, value);
    }
}
...
static esp_err_t PATCH_update_settings(httpd_req_t * req)
{
    ...
    if (cJSON_IsString(item = cJSON_GetObjectItem(root, "sv2Host"))) {
        nvs_config_set_string(NVS_CONFIG_SV2_HOST, item->valuestring);
    }
    if (cJSON_IsString(item = cJSON_GetObjectItem(root, "sv2AuthorityPublicKey"))) {
        nvs_config_set_string(NVS_CONFIG_SV2_AUTHORITY_PUBLIC_KEY, item->valuestring);
    }
    if (cJSON_IsString(item = cJSON_GetObjectItem(root, "fallbackSv2Host"))) {
        nvs_config_set_string(NVS_CONFIG_FALLBACK_SV2_HOST, item->valuestring);
    }
    if (cJSON_IsString(item = cJSON_GetObjectItem(root, "fallbackSv2AuthorityPublicKey"))) {
        nvs_config_set_string(NVS_CONFIG_FALLBACK_SV2_AUTHORITY_PUBLIC_KEY, item->valuestring);
    }
    if (cJSON_IsString(item = cJSON_GetObjectItem(root, "socks5ProxyHost"))) {
        nvs_config_set_string(NVS_CONFIG_SOCKS5_PROXY_HOST, item->valuestring);
    }
    if (cJSON_IsString(item = cJSON_GetObjectItem(root, "socks5ProxyUser"))) {
        nvs_config_set_string(NVS_CONFIG_SOCKS5_PROXY_USER, item->valuestring);
    }
    if (cJSON_IsString(item = cJSON_GetObjectItem(root, "socks5ProxyPass"))) {
        nvs_config_set_string(NVS_CONFIG_SOCKS5_PROXY_PASS, item->valuestring);
    }
    nvs_set_u16_json_clamped(root, "stratumProtocol", NVS_CONFIG_STRATUM_PROTOCOL,
                             STRATUM_PROTOCOL_V1, STRATUM_PROTOCOL_V2);
    nvs_set_u16_json_clamped(root, "fallbackStratumProtocol", NVS_CONFIG_FALLBACK_STRATUM_PROTOCOL,
                             STRATUM_PROTOCOL_V1, STRATUM_PROTOCOL_V2);
    nvs_set_u16_json_clamped(root, "stratumPort", NVS_CONFIG_STRATUM_PORT, 1, UINT16_MAX);
    nvs_set_u16_json_clamped(root, "fallbackStratumPort", NVS_CONFIG_FALLBACK_STRATUM_PORT, 1, UINT16_MAX);
    nvs_set_u16_json_clamped(root, "sv2Port", NVS_CONFIG_SV2_PORT, 1, UINT16_MAX);
    nvs_set_u16_json_clamped(root, "fallbackSv2Port", NVS_CONFIG_FALLBACK_SV2_PORT, 1, UINT16_MAX);
    nvs_set_u16_json_clamped(root, "socks5ProxyEnabled", NVS_CONFIG_SOCKS5_PROXY_ENABLED, 0, 1);
    nvs_set_u16_json_clamped(root, "socks5ProxyPort", NVS_CONFIG_SOCKS5_PROXY_PORT, 1, UINT16_MAX);
    ...
    nvs_set_u16_json_clamped(root, "coreVoltage", NVS_CONFIG_ASIC_VOLTAGE,
                             NVS_CONFIG_ASIC_VOLTAGE_MIN_MV, NVS_CONFIG_ASIC_VOLTAGE_MAX_MV);
    nvs_set_u16_json_clamped(root, "frequency", NVS_CONFIG_ASIC_FREQ,
                             NVS_CONFIG_ASIC_FREQUENCY_MIN_MHZ, NVS_CONFIG_ASIC_FREQUENCY_MAX_MHZ);
    nvs_set_u16_json_clamped(root, "flipscreen", NVS_CONFIG_FLIP_SCREEN, 0, 1);
    (void)cJSON_GetObjectItem(root, "overheat_mode");
    nvs_set_u16_json_clamped(root, "invertscreen", NVS_CONFIG_INVERT_SCREEN, 0, 1);
    nvs_set_u16_json_clamped(root, "invertfanpolarity", NVS_CONFIG_INVERT_FAN_POLARITY, 0, 1);
    nvs_set_u16_json_clamped(root, "autofanspeed", NVS_CONFIG_AUTO_FAN_SPEED, 0, 1);
    nvs_set_u16_json_clamped(root, "fanspeed", NVS_CONFIG_FAN_SPEED,
                             NVS_CONFIG_FAN_SPEED_MIN_PERCENT, NVS_CONFIG_FAN_SPEED_MAX_PERCENT);
    nvs_set_u16_json_clamped(root, "overclockEnabled", NVS_CONFIG_OVERCLOCK_ENABLED, 0, 1);
...
static esp_err_t GET_system_info(httpd_req_t * req)
{
    ...
    char * sv2Host = nvs_config_get_string(NVS_CONFIG_SV2_HOST, CONFIG_SV2_HOST);
    char * sv2AuthorityPublicKey =
        nvs_config_get_string(NVS_CONFIG_SV2_AUTHORITY_PUBLIC_KEY, CONFIG_SV2_AUTHORITY_PUBLIC_KEY);
    char * fallbackSv2Host = nvs_config_get_string(NVS_CONFIG_FALLBACK_SV2_HOST, CONFIG_FALLBACK_SV2_HOST);
    char * fallbackSv2AuthorityPublicKey =
        nvs_config_get_string(NVS_CONFIG_FALLBACK_SV2_AUTHORITY_PUBLIC_KEY,
                              CONFIG_FALLBACK_SV2_AUTHORITY_PUBLIC_KEY);
    char * socks5ProxyHost = nvs_config_get_string(NVS_CONFIG_SOCKS5_PROXY_HOST, CONFIG_SOCKS5_PROXY_HOST);
    char * socks5ProxyUser = nvs_config_get_string(NVS_CONFIG_SOCKS5_PROXY_USER, CONFIG_SOCKS5_PROXY_USER);
    ...
    cJSON_AddNumberToObject(root, "hashRate", SYSTEM_get_current_hashrate(GLOBAL_STATE));
    ...
    cJSON_AddNumberToObject(root, "coreVoltage",
    ...
    cJSON_AddNumberToObject(root, "frequency",
    ...
    cJSON_AddNumberToObject(root, "expectedAsicCount", ASIC_get_expected_asic_count(GLOBAL_STATE));
    cJSON_AddNumberToObject(root, "smallCoreCount", ASIC_get_small_core_count(GLOBAL_STATE));
    cJSON_AddNumberToObject(root, "actualFrequency",
                            GLOBAL_STATE->POWER_MANAGEMENT_MODULE.actual_frequency);
    cJSON_AddNumberToObject(root, "asicJobIntervalMs",
                            (double)dispatch_config.current_interval_us / 1000.0);
    cJSON_AddNumberToObject(root, "asicRawResults",
                            (double)GLOBAL_STATE->ASIC_TASK_MODULE.raw_result_count);
    cJSON_AddNumberToObject(root, "asicDuplicateResults",
                            (double)GLOBAL_STATE->ASIC_TASK_MODULE.duplicate_result_count);
    cJSON_AddNumberToObject(root, "asicJobLookupMisses",
                            (double)GLOBAL_STATE->ASIC_TASK_MODULE.job_lookup_miss_count);
    cJSON_AddNumberToObject(root, "asicValidResults",
                            (double)GLOBAL_STATE->ASIC_TASK_MODULE.valid_result_count);
    cJSON_AddNumberToObject(root, "asicInvalidResults",
                            (double)GLOBAL_STATE->ASIC_TASK_MODULE.invalid_result_count);
    cJSON_AddNumberToObject(root, "asicRetiredJobMatches",
                            (double)GLOBAL_STATE->ASIC_TASK_MODULE.retired_job_match_count);
    ...
    cJSON_AddNumberToObject(root, "stratumProtocol",
                            nvs_config_get_u16_clamped(NVS_CONFIG_STRATUM_PROTOCOL,
                                                       CONFIG_STRATUM_PROTOCOL,
                                                       STRATUM_PROTOCOL_V1,
                                                       STRATUM_PROTOCOL_V2));
    cJSON_AddNumberToObject(root, "fallbackStratumProtocol",
                            nvs_config_get_u16_clamped(NVS_CONFIG_FALLBACK_STRATUM_PROTOCOL,
    ...
    cJSON_AddStringToObject(root, "sv2Host", sv2Host);
    cJSON_AddNumberToObject(root, "sv2Port", nvs_config_get_u16(NVS_CONFIG_SV2_PORT, CONFIG_SV2_PORT));
    cJSON_AddStringToObject(root, "sv2AuthorityPublicKey", sv2AuthorityPublicKey);
    cJSON_AddStringToObject(root, "fallbackSv2Host", fallbackSv2Host);
    cJSON_AddNumberToObject(root, "fallbackSv2Port",
                            nvs_config_get_u16(NVS_CONFIG_FALLBACK_SV2_PORT, CONFIG_FALLBACK_SV2_PORT));
    cJSON_AddStringToObject(root, "fallbackSv2AuthorityPublicKey", fallbackSv2AuthorityPublicKey);
    cJSON_AddNumberToObject(root, "socks5ProxyEnabled",
                            nvs_config_get_u16(NVS_CONFIG_SOCKS5_PROXY_ENABLED, DEFAULT_SOCKS5_PROXY_ENABLED));
    cJSON_AddStringToObject(root, "socks5ProxyHost", socks5ProxyHost);
    cJSON_AddNumberToObject(root, "socks5ProxyPort",
                            nvs_config_get_u16(NVS_CONFIG_SOCKS5_PROXY_PORT, CONFIG_SOCKS5_PROXY_PORT));
    cJSON_AddStringToObject(root, "socks5ProxyUser", socks5ProxyUser);
    ...
    free(sv2Host);
    free(sv2AuthorityPublicKey);
    free(fallbackSv2Host);
    free(fallbackSv2AuthorityPublicKey);
    free(socks5ProxyHost);
    free(socks5ProxyUser);
    free(board_version);
