    ...
    while (1) {
        ...
        if (connected_to_fallback &&
            ...
            if ((int32_t)(now - next_primary_probe_tick) >= 0) {
                ...
                next_primary_probe_tick =
                    now + pdMS_TO_TICKS(PRIMARY_PROBE_INTERVAL_MS);
            }
                                     ...
                                     const stratum_pool_target_t *target)
{
    ...
    if (STRATUM_V1_subscribe(sock,
                             subscribe_message_id,
                             GLOBAL_STATE->asic_model_str) < 0 ||
            STRATUM_V1_authorize(sock,
                                 authorize_message_id,
                                 target->user,
                                 target->password != NULL ? target->password : "") < 0) {
        goto cleanup;
    }
...
cleanup:
    ...
    return subscribe_ok && authorize_ok && notify_seen;
}
